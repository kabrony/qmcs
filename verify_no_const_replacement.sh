#!/usr/bin/env bash
#
# verify_no_const_replacement.sh
# 1) Overwrites fix_and_run.sh (removing any const->var lines)
# 2) Overwrites fix_common_syntax_issues.sh (no const->var replacements)
# 3) Manually runs fix_common_syntax_issues.sh
# 4) Then runs unify_build_run_check.sh
# 
# Usage:
#   chmod +x verify_no_const_replacement.sh
#   ./verify_no_const_replacement.sh

LOGFILE="verify_no_const_replacement.log"
echo "[INFO] Starting verify_no_const_replacement.sh..." | tee "$LOGFILE"

################################################################################
# STEP 1: Overwrite fix_and_run.sh WITHOUT const->var lines
################################################################################
echo "[INFO] Overwriting fix_and_run.sh with corrected content..." | tee -a "$LOGFILE"
cat << 'FIXRUN' > fix_and_run.sh
#!/usr/bin/env bash
...
echo "[INFO] fix_and_run.sh has been overwritten." | tee -a "$LOGFILE"

################################################################################
# STEP 2: Overwrite fix_common_syntax_issues.sh (no const->var replacements)
################################################################################
echo "[INFO] Overwriting fix_common_syntax_issues.sh with corrected content..." | tee -a "$LOGFILE"
cat << 'FIXCOMMON' > fix_common_syntax_issues.sh
#!/usr/bin/env bash
...
chmod +x fix_common_syntax_issues.sh
echo "[INFO] fix_common_syntax_issues.sh has been overwritten." | tee -a "$LOGFILE"

################################################################################
# STEP 3: Manually run fix_common_syntax_issues.sh
################################################################################
echo "[INFO] Manually running fix_common_syntax_issues.sh now..." | tee -a "$LOGFILE"
./fix_common_syntax_issues.sh 2>&1 | tee -a "$LOGFILE"

################################################################################
# STEP 4: Run unify_build_run_check.sh
################################################################################
if [[ -f "./unify_build_run_check.sh" ]]; then
  echo "[INFO] Running unify_build_run_check.sh to rebuild & up containers..." | tee -a "$LOGFILE"
  chmod +x unify_build_run_check.sh
  ./unify_build_run_check.sh 2>&1 | tee -a "$LOGFILE"
else
  echo "[WARN] unify_build_run_check.sh not found. Please run docker-compose build/up manually." | tee -a "$LOGFILE"
...
echo "[INFO] Finished verify_no_const_replacement.sh. Check '$LOGFILE' for details."
